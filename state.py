...
class State(object):
    ...
    @classmethod
    def from_sentence(cls, tokens: Sequence[Token]) -> "State":
        """Build the initial arc-eager state from a tokenized sentence."""
        if not tokens:
            raise ValueError("A non-empty token sequence is required to initialize a State")
        if tokens[0].id != 0:
            raise ValueError("The first token must be the artificial ROOT with id 0")
        stack = [tokens[0]]
        buffer = list(tokens[1:])
        arcs: Set[Tuple[int, str, int]] = set()
        return cls(stack, buffer, arcs)
    
    def is_final(self) -> bool:
        """Return True when the buffer is empty, i.e., no unread tokens remain."""
        return len(self.B) == 0
