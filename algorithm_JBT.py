"""Arc-eager transition system implementation and documentation."""
...
class Sample(object):
    ...
    def state_to_feats(self, nbuffer_feats: int = 2, nstack_feats: int = 2):
        ...
        def form_or_pad(token: Optional[Token]) -> str:
            return token.form if token else pad_token

        def upos_or_pad(token: Optional[Token]) -> str:
            return token.upos if token else pad_token

        stack_tokens: List[Optional[Token]] = []
        if nstack_feats:
            stack_slice = self.state.S[-nstack_feats:]
            stack_tokens = [None] * (nstack_feats - len(stack_slice)) + stack_slice

        buffer_tokens: List[Optional[Token]] = []
        if nbuffer_feats:
            buffer_slice = self.state.B[:nbuffer_feats]
            buffer_tokens = buffer_slice + [None] * (nbuffer_feats - len(buffer_slice))

        words = [form_or_pad(tok) for tok in stack_tokens] + [form_or_pad(tok) for tok in buffer_tokens]
        upos = [upos_or_pad(tok) for tok in stack_tokens] + [upos_or_pad(tok) for tok in buffer_tokens]
...
class ArcEager():
    ...
    def LA_is_valid(self, state: State) -> bool:
        ...
        """
        if not (state.S and state.B):
            return False
        top_stack = state.S[-1]
        if top_stack.id == 0:
            return False
        return not self.has_head(top_stack, state.A)
    ...
    def LA_is_correct(self, state: State) -> bool:
        ...
        if not self.LA_is_valid(state):
            return False
        stack_top = state.S[-1]
        buffer_head = state.B[0]
        return stack_top.head == buffer_head.id
    ...
    def RA_is_correct(self, state: State) -> bool:
        ...
        stack_top = state.S[-1]
        buffer_head = state.B[0]
        return buffer_head.head == stack_top.id
    ...
    def RA_is_valid(self, state: State) -> bool:
        ...
        if not (state.S and state.B):
            return False
        buffer_head = state.B[0]
        return not self.has_head(buffer_head, state.A)
    ...
    def REDUCE_is_correct(self, state: State) -> bool:
        ...
        if not self.REDUCE_is_valid(state):
            return False
        stack_top = state.S[-1]
        return all(token.head != stack_top.id for token in state.B)
    ...
    def REDUCE_is_valid(self, state: State) -> bool:
        ...
        if stack_top.id == 0:
            return False
        return self.has_head(stack_top, state.A)
    ...
    @staticmethod
    def has_head(token: Optional[Token], arcs: Sequence[tuple[int, str, int]]) -> bool:
        """Utility that checks whether a token already has a head assigned in the generated arc set."""
        if token is None:
            return False
